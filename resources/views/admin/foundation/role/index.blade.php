				...
				<div class="ibox-content">
				    <div class="row">
    					...
    					<div class="col-sm-3 pull-right">
    					   <a href="{{ U('Foundation/Role/create')}}" class="btn btn-sm btn-primary pull-right">添加</a>
    					</div>
						@endif
								...
								<td>
									<div class="btn-group">
										...
										<ul class="dropdown-menu">
										    ...
										    <li><a href="{{ U('Foundation/Role/update')}}?id={{ $item->id }}" class="font-bold">修改</a></li>
											@endif

											@if(role('Foundation/Role/auth'))
											<li><a href="{{ U('Foundation/Role/auth')}}?id={{ $item->id }}" class="font-bold">授权</a></li>
											@endif

											@if(role('Foundation/Role/status'))
												<li class="divider"></li>
												@if($item->status == 1)
												<li><a href="{{ U('Foundation/Role/status')}}?id={{ $item->id }}&status=0" >禁用</a></li>
												@else
												<li><a href="{{ U('Foundation/Role/status')}}?id={{ $item->id }}&status=1" >启用</a></li>
												@endif
											@endif

											@if(role('Foundation/Role/destroy'))
											<li class="divider"></li>
											<li><a href="{{ U('Foundation/Role/destroy')}}?id={{ $item->id }}" onclick="return confirm('你确定执行删除操作？');">删除</a></li>
											@endif
										</ul>
									</div>
								</td>
