...
use App\Services\User\Department;
use Request;
...
class RoleController extends Controller
{
    ...
    function index()
    {
        ...
        return view('admin.foundation.role.index', compact('list'));
    }
    ...
    public function create()
    {
        if(Request::method() == 'POST'){
            ...
            if($this->_service->create(Request::input('info'))){
                $this->showMessage('操作成功', U( 'Foundation/Role/index'));
            }else{
                $this->showMessage('操作失败', U( 'Foundation/Role/index'));
            }
        }
        $level = $this->level;
        return view('admin.foundation.role.edit', compact('level', 'Departments'));
    }
    ...
    public function update()
    {
        ...
        $level = $this->level;
        return view('admin.foundation.role.edit', compact('data', 'level', 'Departments'));
    }
    ...
    public function auth()
    {
        ...
        $list =  session(LOGIN_MARK_SESSION_KEY);
        //为ztree做数据准备
        $zTree = []; $all = [];
        foreach($this->_user['menus'] as $key => $value)
        {
        ...
        return view('admin.foundation.role.auth', compact('data','zTree','all'));
    }
