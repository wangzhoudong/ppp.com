...
class ToolController extends Controller
{
    ...
    function specialupload() {
        ...
        return view('admin.foundation.tool.specialupload', compact('data'));
    }
    ...
    function alimanage()
    {
        ...
        //构建路径
        $folderPath = '/<a href="' . U( 'Foundation/Tool/alimanage') . '?folder=">根目录</a>/';
        if($folder){
            $dirPath = '';
            $dirs = array_filter(explode('/', $folder));
            foreach ($dirs AS $dir){
                $dirPath .= $dir . '/';
                $folderPath .= '<a href="' . U( 'Foundation/Tool/alimanage') . '?folder=' . $dirPath . '">'. $dir .'</a>/';
            }
        }
        ...
        return view('admin.foundation.tool.alimanage', compact('item', 'img_path', 'file_path', 'folderPath', 'uploadControl'));
    }
    ...
    private function _createDir($dirname, $folder)
    {
        if(strpos($dirname, '/') === false && preg_match('/^([0-9a-zA-Z\_]+)$/is', $dirname)){
            ...
            if($code == null){
                $this->showMessage('目录创建成功！', U( 'Foundation/Tool/alimanage') . '?folder=' . $newdir . '/');
            }else{
