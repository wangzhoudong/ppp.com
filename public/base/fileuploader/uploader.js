...
function upload(obj){
    this.upobj = {
        ...
        UploaderServer : obj.UploaderServer ? obj.UploaderServer :"/api/attachment/webupload?elementid=&folder=&_time=" + Math.random() ,
...
upload.prototype.upLoader=function(){
    ...
    var uploader = WebUploader.create({
        // swf文件路径
        swf:'/base/fileuploader/uploader.swf',
...
//  });
    ...
    uploader.on("fileQueued",function(file,percentage){
        ...
        var $li = '<li data-uploaderId="'+file.id+'">\
                        <em class="close">×</em>\
                        <a class="cc_file" href="javascript:;" title="" target="_blank" > <img src="/base/fileuploader/img/file.jpg" alt="" /></a>\
                        <p class="rate"><span></span></p>\
                        <p class="yes">上传成功</p>\
                        <p class="no">上传失败</p>\
                    </li>';
        if(_this.upobj.UploaderSingle) {
            if($("#rt_"+file.source.ruid).parent().parent().parent().children("li").eq(0).find("a.cc_file").attr('href')){
    ...
    uploader.on('uploadSuccess', function( file, response ) {
        ...
        if(response.code==200){
            ...
            if(_this.upobj.UploaderSingle) {
                ruid.find("li[data-uploaderId='"+file.id+"'] a.cc_file").attr("href",response.fileurl);
                ruid.find("li[data-uploaderId='"+file.id+"'] a.cc_file").attr("title",response.filename);
                ruid.find("li[data-uploaderId='"+file.id+"'] a img").attr("alt",response.filename);
